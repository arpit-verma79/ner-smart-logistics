...
CARTO_API_KEY = os.getenv("CARTO_API_KEY", "cb1_3qjl_1_3d76cd823204a9179cff65ef")
...
.official-footer a { color: #b9d6cf; text-decoration: none; }
.official-footer a:hover, .official-footer a:focus-visible { color: #f4c95d; text-decoration: underline; }
...
.portal-nav { display: flex; gap: 8px; padding: 8px 14px; background: #171e24; color: #e8eeeb; font-size: .82rem; flex-wrap: wrap; align-items: center; }
.portal-nav a { color: #e8eeeb; opacity: .92; text-decoration: none; padding: 7px 10px; border-radius: 5px; transition: background .2s ease, color .2s ease; }
.portal-nav a:hover, .portal-nav a:focus-visible { background: rgba(216,173,111,.16); color: #f4c95d; outline: none; }
.portal-nav .portal-nav-admin { margin-left: auto; color: #d8ad6f; border-left: 1px solid rgba(205,214,209,.2); padding-left: 18px; }
.admin-toolbar { display: flex; justify-content: space-between; align-items: center; gap: 16px; margin: 0 0 16px; padding: 12px 16px; border: 1px solid rgba(167,214,198,.2); border-radius: 8px; background: rgba(18,28,31,.82); }
.admin-toolbar strong { color: #d8e5df; }
.admin-toolbar span { color: #a7b5b7; font-size: .82rem; }
...
@media (max-width: 800px) { .govt-header-content { grid-template-columns: 1fr auto; } .govt-brand-center { display: none; } .govt-search input { min-width: 0; width: 42px; } .portal-nav { gap: 4px; } .portal-nav .portal-nav-admin { margin-left: 0; border-left: 0; padding-left: 10px; } .portal-updates { align-items: flex-start; flex-wrap: wrap; } .portal-updates a { margin-left: 0; } }
@media (max-width: 640px) { .admin-toolbar { align-items: flex-start; flex-direction: column; gap: 4px; } }
</style>
"""
...
portal_admin_link = (
    '<a class="portal-nav-admin" href="#admin-operations">Admin workspace</a>'
...
portal_navigation = (
    ...
    '<a href="#portal-about">About the Platform</a>'
    '<a href="#portal-notices">Field Updates</a>'
    '<a href="#portal-insights">Regional Insights</a>'
...
st.sidebar.markdown(
    f'<div style="color:#a7d6c6; font-size:.78rem; margin: 0 0 8px;">Signed in as <b>{escape(st.session_state.role or "field-official")}</b></div>',
    unsafe_allow_html=True,
)
if st.sidebar.button("Sign out", use_container_width=True, key="sign_out_button"):
    st.session_state.logged_in = False
    st.session_state.role = None
    st.session_state.login_page = "home"
    st.rerun()
...
if analyze_clicked:
    st.session_state.route_review_requested = True
# =========================================================
# HELP & SUPPORT
# =========================================================
...
# Contact Support
st.sidebar.markdown(
    '<a href="mailto:support@ner-logistics.gov.in?subject=NER%20Logistics%20Support" '
    'style="display:block; width:100%; box-sizing:border-box; padding:10px; text-align:center; '
    'border:1px solid #52666c; border-radius:8px; color:#f3f7f4; text-decoration:none;">'
    '✉️ Contact Support</a>',
    unsafe_allow_html=True
)
...
st.markdown(
    f"""
    <div id="portal-about"></div>
    <div id="portal-home" class="command-header">
...
if st.session_state.pop("route_review_requested", False):
    st.success("Corridor review refreshed. The live route map and advisory are ready below.")
...
with tab_map:
    st.markdown('<div id="portal-insights"></div>', unsafe_allow_html=True)
...
with tab_report:
    ...
    with rep_col2:
        ...
        if not st.session_state.get("camera_attempted", False):
            ...
            if st.button("Close camera", key="close_incident_camera"):
...
if tab_admin is not None:
    with tab_admin:
        st.markdown('<div id="admin-operations"></div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="admin-toolbar"><strong>Administrator workspace</strong><span>Review, publish, and maintain field intelligence records.</span></div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            """
        ...
        with filter_col1:
            admin_status = st.selectbox("Review status", ["All", "Pending", "Approved", "Rejected"], key="admin_status_filter")
            admin_state = st.selectbox("State", ["All", "Assam", "Arunachal Pradesh", "Manipur", "Meghalaya", "Mizoram", "Nagaland", "Sikkim", "Tripura", "Uttar Pradesh"], key="admin_state_filter")
        with filter_col2:
            admin_district = st.selectbox("District / hub", ["All"] + list(CITY_COORDS.keys()), key="admin_district_filter")
            admin_category = st.selectbox("Category", ["All", "Road condition", "Farmer support", "Survey / inspection", "Emergency response"], key="admin_category_filter")
        with filter_col3:
            admin_search = st.text_input("Search submissions", placeholder="Location, category, description, or submitter", key="admin_report_search")
            ...
            with st.expander(f"#{admin_id} · {admin_type} · {admin_row_status} · {admin_time}"):
                ...
                with action_col2:
                    if st.button("Reject", key=f"reject_{admin_id}"):
                        ...
                        st.rerun()
                with action_col3:
                    if st.button("Save changes", key=f"save_{admin_id}"):
...
st.markdown(
    ...
    <footer class="official-footer">
        <div class="official-footer-grid">
            ...
            <section id="portal-resources">
                <h3>Important Links</h3>
                <ul>
                    <li><a href="https://mdoner.gov.in/" target="_blank" rel="noopener">Department portal</a></li>
                    <li><a href="#portal-insights">Regional transport advisories</a></li>
                    <li><a href="#portal-notices">Citizen services</a></li>
                    <li><a href="#portal-resources">Open data catalogue</a></li>
                </ul>
            </section>
            <section>
                <h3>Support</h3>
                <ul>
                    <li><a href="mailto:support@ner-logistics.gov.in?subject=Help%20and%20accessibility">Help and accessibility</a></li>
                    <li><a href="#portal-contact">Privacy policy</a></li>
                    <li><a href="#portal-contact">Terms and conditions</a></li>
                    <li><a href="#portal-home">Sitemap</a></li>
                </ul>
            </section>
            <section id="portal-contact">
                <h3>Contact</h3>
                <p>Regional Operations Helpdesk<br><a href="mailto:support@ner-logistics.gov.in">support@ner-logistics.gov.in</a><br>Mon-Fri, 09:00-18:00 IST</p>
                <p>Follow official updates: <a href="#portal-notices">@NERLogistics</a></p>
            </section>
        </div>
